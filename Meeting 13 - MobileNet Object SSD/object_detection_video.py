...
while True:
    ret, frame = cap.read() #reads frame from video stream, ret is boolean that returns True if the frame successfully read
    ...
    blob = cv2.dnn.blobFromImage(cv2.resize(frame, (300, 300)), 0.007843, (300, 300),
                                 127.5)  # Carry out the object detection process (same as the previous face detection project)
    net.setInput(blob)
